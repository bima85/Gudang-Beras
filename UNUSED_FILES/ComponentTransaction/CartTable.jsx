...
export default function CartTable({
    ...
    setCartsState = () => {},
    setCartsTotalState = () => {},
    formatPrice = (value) => `Rp ${Number(value || 0).toLocaleString("id-ID")}`,
}) {
    // Jika cartsToShow undefined atau null, gunakan array kosong
    const items = cartsToShow || [];
    return (
                ...
                <Table className="w-full min-w-[600px] text-sm">
                    ...
                    <Tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {items.map((item, index) => {
                            ...
                            const konversi = item.unit?.conversion_to_kg
                                ...
                                : 1;
                            const subtotal =
                                (item.quantity || 0) * konversi * harga_satuan;
                            return (
                                <Tr
                                    key={item.id || index}
                    ...
                    <Tfoot className="bg-gray-100 dark:bg-gray-700">
                        <Tr>
                            <Td
                                colSpan={7}
                                className="text-right font-semibold py-3 px-4"
                            >
                                Total
                            </Td>
                            <Td
                                colSpan={2}
                                className="text-right font-bold text-teal-600 dark:text-teal-400 py-3 px-4"
                            >
                                {formatPrice(
                                    items.reduce((sum, item) => {
                                        const harga_satuan = item.price || 0;
                                        const konversi = item.unit
                                            ?.conversion_to_kg
                                            ? parseFloat(
                                                  item.unit.conversion_to_kg
                                              )
                                            : 1;
                                        return (
                                            sum +
