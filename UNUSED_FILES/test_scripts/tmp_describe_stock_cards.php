...
echo "COLUMNS:\n";
foreach ($pdo->query('SHOW FULL COLUMNS FROM stock_cards') as $r) {
    echo json_encode($r) . "\n";
}

echo "\nSHOW CREATE TABLE:\n";
foreach ($pdo->query('SHOW CREATE TABLE stock_cards') as $r) {
    echo $r['Create Table'] . "\n";
}

echo "\nSAMPLE ROWS (last 5):\n";
foreach ($pdo->query('SELECT * FROM stock_cards ORDER BY id DESC LIMIT 5') as $r) {
    echo json_encode($r) . "\n";
}
